...
-- Check icon with no (or invalid) extension matching :: if NOT supported_icon_formats[icon_file_ext]
    it('finds icons without specified path or extension', function()
        ...
        assert_found_in_path('icon6', './spec/menubar/home/.icons/icon6.xpm') -- Similar to original tests + testing xpm extension
        assert_found_in_path('icon7', './spec/menubar/home/.icons/icon7.svg') -- Similar to original tests + testing svg extension
        ...
        assert.is_false(utils.lookup_icon_uncached('png')) -- file does not exist
        assert.is_false(utils.lookup_icon_uncached('icon8')) -- file does not exist
    end)
end)
